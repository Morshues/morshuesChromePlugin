...
function scan_max(targetNode) {
  const comments = Array.from(document.querySelectorAll('[role=article] span[lang]'))
  ...
  const count_array = [null, 0, 0, 0, 0];
  for (let i = 0; i < comments.length; i++) {
    const content_block = comments[i];
    if (content_block == null) {
      continue;
    }
    const value = parseInt(content_block.textContent);
    count_array[value]++;
  }
  let max_index = null;
  let temp_value = 1;
...
function auto_answer() {
  const quiz_block = document.getElementsByClassName('anime-quiz')[0]
  if (quiz_block != null) {
    const quiz_btns = quiz_block.getElementsByTagName('a');
